...
`metadata["source_url"]` is REQUIRED on every call: test providers use it to decide what to
answer, and logging/caching key off it. Providers raise `ProviderTimeout` or `ProviderError`;
they never return partial output.
...
loaded instead of the built-ins (used to inject test providers).
"""
